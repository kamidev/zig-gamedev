...
pub fn getPkg(b: *std.build.Builder, options_pkg: std.build.Pkg) std.build.Pkg {
    const pkg = std.build.Pkg{
        .name = "zxaudio2",
        .path = .{ .path = comptime thisDir() ++ "/src/zxaudio2.zig" },
    ...
    if (enable_debug_layer) {
        exe.step.dependOn(
            &exe.builder.addInstallFile(
                .{ .path = comptime thisDir() ++ "/../zwin32/bin/x64/xaudio2_9redist_debug.dll" },
        ...
        exe.step.dependOn(
            &exe.builder.addInstallFile(
                .{ .path = comptime thisDir() ++ "/../zwin32/bin/x64/xaudio2_9redist.dll" },
                "bin/xaudio2_9redist.dll",
            ).step,
        );
    }
